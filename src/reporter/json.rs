...
impl BenchReporter for JsonReporter {
    fn print(&self, w: &mut dyn Write, report: &BenchReport) -> anyhow::Result<()> {
        let report = build_serde_report(report);
...
/// Build a serde-compatible report from the given benchmark report.
pub fn build_serde_report(report: &BenchReport) -> Report {
...
/// A serde-compatible report for benchmark results.
#[derive(Serialize)]
pub struct Report {
    summary: Summary,
    #[serde(skip_serializing_if = "Option::is_none")]
